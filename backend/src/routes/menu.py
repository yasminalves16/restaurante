...
def get_menu():
    """Obter cardápio filtrado por tipo (delivery/local) e categoria"""
    order_type = request.args.get('type', 'local')  # 'delivery' ou 'local'
...
def update_menu_item(item_id):
    ...
    try:
        ...
        if 'available_for_delivery' in data:
            item.available_for_delivery = data['available_for_delivery']
        if 'available_for_local' in data:
            item.available_for_local = data['available_for_local']
